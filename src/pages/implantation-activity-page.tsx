    ...
    <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div className="space-y-4">
        ...
        <Card className="shadow-none"><CardHeader><CardTitle>Ações de status</CardTitle></CardHeader><CardContent className="flex flex-wrap gap-2">
          {capabilities.blockActivity && activity.status !== "BLOQUEADO" && !["CONCLUIDO", "NAO_APLICAVEL", "CANCELADO"].includes(activity.status) ? <Button variant="outline" onClick={() => invokeReason("Bloquear atividade", (reason) => repository.blockActivity({ activityId: id, version: activity.version, reason, requestId: createImplantationRequestId() }))}><Lock />Bloquear</Button> : null}
          {capabilities.unblockActivity && activity.status === "BLOQUEADO" ? <Button variant="outline" onClick={() => invokeReason("Desbloquear atividade", (reason) => repository.unblockActivity({ activityId: id, version: activity.version, reason, requestId: createImplantationRequestId() }))}><Unlock />Desbloquear</Button> : null}
          {capabilities.markNotApplicable && activity.status !== "BLOQUEADO" && !["CONCLUIDO", "NAO_APLICAVEL", "CANCELADO"].includes(activity.status) ? <Button variant="outline" onClick={() => invokeReason("Marcar como não aplicável", (reason) => repository.markNotApplicable({ activityId: id, version: activity.version, reason, requestId: createImplantationRequestId() }))}><CircleOff />Não aplicável</Button> : null}
          {capabilities.completeActivity && activity.status === "EM_ANDAMENTO" ? <Button onClick={() => void run(() => repository.completeActivity({ activityId: id, version: activity.version, observation: observation || "Atividade concluída.", requestId: createImplantationRequestId() }))}><Check />Concluir</Button> : null}
