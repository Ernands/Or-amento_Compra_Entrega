...
import { ItemEditSheet } from "@/components/app/item-edit-sheet";
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden py-0 shadow-none">
        <CardContent className="overflow-x-auto p-0">
          <Table>
            <TableHeader><TableRow><TableHead className="pl-6">ID interno</TableHead><TableHead>Código</TableHead><TableHead>Item</TableHead><TableHead>Grupo / Área</TableHead><TableHead>Definição</TableHead><TableHead className="w-24"><span className="sr-only">Ações</span></TableHead></TableRow></TableHeader>
            <TableBody>
              {filtered.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{canEdit ? <Button variant="ghost" size="sm" onClick={() => setEditingItem(item)}><Pencil />Editar</Button> : null}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      {editingItem ? <ItemEditSheet key={`${editingItem.id}:${editingItem.version}`} item={editingItem} open onOpenChange={(open) => { if (!open) setEditingItem(null); }} onSave={updateItem} /> : null}
    </div>
  );
}
