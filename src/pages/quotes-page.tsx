...
function formatPaymentTerms(quote: Quote): string {
  const method = quote.paymentMethod || "—";
  if (!quote.installments) return method;
  const installments = `${Math.max(1, quote.installments || 1)}x`;
  return `${method} · ${installments}${quote.hasDownPayment ? " · com entrada" : " · sem entrada"}`;
}
