...
function Comparison(props: { groups: Array<[string, Quote[]]>; supplierMap: Map<string, { name: string; rating?: number | null }>; storeMap: Map<string, { name: string }>; itemMap: Map<string, { name: string; operationalCode: string }>; canSelect: boolean; selectingId: string; onSelect: (quote: Quote) => void }) {
  ...
  return <div className="space-y-4">{props.groups.map(([necessityId, quotes]) => { const first = quotes[0]; const minTotal = Math.min(...quotes.map((quote) => quote.total)); const minLead = Math.min(...quotes.map((quote) => quote.leadTimeDays)); const maxRating = Math.max(...quotes.map((quote) => quote.supplierRating ?? -1)); return <Card key={necessityId} className="shadow-none"><CardHeader><CardTitle className="text-base">{props.itemMap.get(first.itemId)?.name}</CardTitle><CardDescription>{necessityId} · {props.storeMap.get(first.storeId)?.name} · {props.itemMap.get(first.itemId)?.operationalCode}</CardDescription></CardHeader><CardContent className="grid gap-3 xl:grid-cols-3">{quotes.map((quote) => { const supplier = props.supplierMap.get(quote.supplierId); return <div key={quote.id} className={`rounded-xl border p-4 ${quote.selected ? "border-emerald-300 bg-emerald-50/60" : ""}`}><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{supplier?.name || quote.supplierId}</p><p className="font-mono text-xs text-muted-foreground">{quote.id}</p></div>{quote.selected ? <StatusBadge status="SELECIONADA" /> : null}</div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><ComparisonValue icon={<Truck />} label="Total" value={currencyFormatter.format(quote.total)} highlight={quote.total === minTotal} /><ComparisonValue icon={<Timer />} label="Prazo" value={`${quote.leadTimeDays} dia(s)`} highlight={quote.leadTimeDays === minLead} /><ComparisonValue icon={<Star />} label="Nota" value={quote.supplierRating === null ? "—" : quote.supplierRating.toLocaleString("pt-BR")} highlight={maxRating >= 0 && quote.supplierRating === maxRating} /><div><p className="text-xs text-muted-foreground">Pagamento</p><p className="font-medium">{quote.paymentMethod || "—"}</p></div></div><p className="mt-3 text-xs text-muted-foreground">Unitário: {currencyFormatter.format(quote.unitPrice)} · Frete: {currencyFormatter.format(quote.freight)} · Outros: {currencyFormatter.format(quote.otherCosts)}</p>{props.canSelect && quote.status === "RECEBIDA" && !quote.selected ? <Button className="mt-4 w-full" variant="outline" disabled={props.selectingId === quote.id} onClick={() => props.onSelect(quote)}>Selecionar proposta</Button> : null}</div>; })}</CardContent></Card>; })}</div>;
}
