...
export function QuotesPage() {
  ...
  const { quotes, suppliers, routes, options, permissions, schemaMode } = state.data;
  ...
  const filtered = quotes.filter((quote) => {
    ...
    const storesText = quote.storeIds.map((id) => storeMap.get(id)?.name || id).join(" ");
    return (!storeFilter || quote.storeIds.includes(storeFilter))
      && (!statusFilter || quote.status === statusFilter)
      && (!supplierFilter || quote.supplierId === supplierFilter)
      && (!normalizedQuery || normalize([quote.id, ...quote.necessityIds, item?.name, item?.operationalCode, storesText, supplier?.name].join(" ")).includes(normalizedQuery));
  });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<div className="flex flex-wrap gap-2">{permissions.createSupplier ? <Button variant="outline" onClick={() => setSupplierSheet(true)}><PackagePlus />Fornecedor</Button> : null}{permissions.create && groupedReady ? <Button onClick={() => setQuoteSheet({})}><Plus />Nova proposta</Button> : null}</div>}
      />
      {!groupedReady ? <Alert><AlertDescription><strong>Pré-migração — somente leitura.</strong> O backend novo já está ativo, mas nenhuma proposta será gravada no formato agrupado até a execução manual de <code>migrateQuoteProposalsV1()</code>.</AlertDescription></Alert> : null}
      ...
      {!quotes.length ? <EmptyQuotes canCreate={permissions.create && groupedReady} canCreateSupplier={permissions.createSupplier} onNewSupplier={() => setSupplierSheet(true)} onNewQuote={() => setQuoteSheet({})} /> : view === "list" ? (
        <ProposalTable publicView={publicView} quotes={visible} supplierMap={supplierMap} storeMap={storeMap} itemMap={itemMap} canEdit={permissions.edit && groupedReady} canDelete={permissions.delete && groupedReady} canSelect={permissions.select && groupedReady} selectingId={selectingId} deletingId={deletingId} reopeningId={reopeningId} onEdit={(quote) => setQuoteSheet({ quote })} onReopen={reopenReceivedQuote} onDelete={removeQuote} onSelect={chooseQuote} />
...
function groupComparableProposals(quotes: Quote[]): Array<[string, Quote[]]> {
  ...
  quotes.forEach((quote) => groups.set(quote.scopeSignature, [...(groups.get(quote.scopeSignature) || []), quote]));
