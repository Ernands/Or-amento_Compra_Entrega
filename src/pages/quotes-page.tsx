  ...
  return (
    <div className="space-y-6">
      ...
      {!quotes.length ? <EmptyQuotes canCreate={permissions.create} canCreateSupplier={permissions.createSupplier} onNewSupplier={() => setSupplierSheet(true)} onNewQuote={() => setQuoteSheet({})} /> : view === "list" ? (
        <QuoteTable publicView={publicView} quotes={visible} supplierMap={supplierMap} storeMap={storeMap} itemMap={itemMap} canEdit={permissions.edit} canDelete={permissions.delete} canSelect={permissions.select} selectingId={selectingId} deletingId={deletingId} onEdit={(quote) => setQuoteSheet({ quote })} onDelete={removeQuote} onSelect={chooseQuote} />
      ) : <Comparison publicView={publicView} groups={grouped} supplierMap={supplierMap} storeMap={storeMap} itemMap={itemMap} selectingId={selectingId} canSelect={permissions.select} onSelect={chooseQuote} />}
...
function Comparison(props: { publicView: boolean; groups: Array<[string, Quote[]]>; supplierMap: Map<string, { name: string; rating?: number | null }>; storeMap: Map<string, { name: string }>; itemMap: Map<string, { name: string; operationalCode: string }>; canSelect: boolean; selectingId: string; onSelect: (quote: Quote) => void }) {
