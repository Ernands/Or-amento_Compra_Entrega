    ...
    <Card className="shadow-none"><CardContent className="space-y-4 p-5">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Input type="date" value={effectiveDate} onChange={(event) => setPlannedDate(event.target.value)} className="sm:w-44" disabled={!capabilities.setOpeningDate || busy} />
          {!implantation ? <Button onClick={defineDate} disabled={!capabilities.setOpeningDate || !effectiveDate || busy}><CalendarClock />Salvar data</Button> : <Button onClick={() => void previewDate()} disabled={!capabilities.setOpeningDate || !effectiveDate || effectiveDate === implantation.plannedOpeningDate || busy}><CalendarClock />Calcular prévia</Button>}
