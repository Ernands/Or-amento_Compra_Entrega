...
function implantationStatusLabelV1(status) {
    const labels = {
...
function mapImplantationStoreV1(table, row) {
    return {
...
function mapImplantationCycleV1(table, row) {
    return {
...
function mapImplantationActivityV1(table, row) {
    return {
...
function implantationDateTimeV1(table, row, header) {
    const index = table.normalizedHeaders.indexOf(normalizeHeader(header));
    if (index < 0 || !row[index])
        return "";
    return row[index] instanceof Date ? row[index].toISOString() : String(row[index]);
}
function currentImplantationForStoreV1(table, storeId) {
...
function activeActivitiesForImplantationV1(table, implantationId) {
    return table.rows.filter((row) => isActiveImplantationRowV1(table, row) && cell(table, row, "ID_Implantacao") === implantationId);
}
function implantationSummaryV1(activityTable, rows) {
...
function buildImplantationOverviewV1(spreadsheet, user) {
    ...
    return {
        ...
        totals: {
            ...
            withOpeningDate: storeItems.filter((entry) => Boolean(entry.store.plannedOpeningDate)).length,
            ...
            blocked: storeItems.reduce((sum, entry) => sum + Number(entry.summary.blocked || 0), 0),
            completedActivities: storeItems.reduce((sum, entry) => sum + Number(entry.summary.completed || 0), 0),
        },
        evidenceValidationPending: true,
    };
}
function buildImplantationChecklistsV1(spreadsheet, user) {
    const overview = buildImplantationOverviewV1(spreadsheet, user);
    return { ...overview, stores: overview.stores.filter((entry) => Boolean(entry.implantation)) };
}
function buildImplantationPendenciesV1(spreadsheet, user) {
    assertImplantationViewV1(spreadsheet, user);
    ...
    const allowedCycleIds = {};
    cycles.rows.forEach((row) => {
        if (isActiveImplantationRowV1(cycles, row) && storeMap[cell(cycles, row, "ID_Loja")])
            allowedCycleIds[cell(cycles, row, "ID_Implantacao")] = true;
    });
    ...
    const items = activities.rows.filter((row) => isActiveImplantationRowV1(activities, row)
        ...
        .filter((entry) => {
        const status = String(entry.activity.status);
        const target = String(entry.activity.currentTargetDate || "");
        return status === "BLOQUEADO" || (!(["CONCLUIDO", "NAO_APLICAVEL", "CANCELADO"].indexOf(status) >= 0) && Boolean(target) && target <= today)
            || (!(["CONCLUIDO", "NAO_APLICAVEL", "CANCELADO"].indexOf(status) >= 0) && !entry.activity.responsibleUserId);
    });
...
function buildImplantationStoreDetailV1(spreadsheet, user, payload) {
    assertImplantationViewV1(spreadsheet, user);
...
function implantationUserAllowsStoreV1(table, row, storeId) {
...
function buildImplantationActivityDetailV1(spreadsheet, user, payload) {
    assertImplantationViewV1(spreadsheet, user);
    ...
    assertStoreScope(user, storeId);
    const storeDetail = buildImplantationStoreDetailV1(spreadsheet, user, { storeId });
...
function buildImplantationTimelineV1(spreadsheet, user, payload) {
    ...
    const matchingRows = activityValues.reduce((result, row, index) => {
        var _a;
        const active = String(((_a = activeValues[index]) === null || _a === void 0 ? void 0 : _a[0]) || "");
        if (String(row[0] || "").trim() === activityId && (!active || isYes(active)))
            result.push(dataStart + index);
        return result;
    }, []).reverse();
...
function buildImplantationMasterChecklistV1(spreadsheet, user) {
    ...
    if (published.length !== 1)
        throw new ApiException("MODEL_NOT_READY", "Deve existir exatamente um Checklist Mestre publicado e ativo.");
...
function requireImplantationRequestIdV1(payload) {
...
function readImplantationUpdateIndexV1(spreadsheet) {
    ...
    idValues.forEach((value, index) => {
        ...
        setCell(table, row, "Request_ID", String(((_a = requestValues[index]) === null || _a === void 0 ? void 0 : _a[0]) || "").trim());
        table.rows.push(row);
        table.rowNumbers.push(dataStart + index);
    });
    return table;
}
function columnLetterV1(columnCount) {
...
function validateOpeningDateV1(value) {
    const date = requireString(value, "plannedOpeningDate");
    calculateImplantationTargetDateV1(date, 0);
    return date;
}
function assertStoreOperationalForImplantationV1(table, row) {
    const status = normalizeHeader(cell(table, row, "Status"));
    if (status !== "ativa")
        throw new ApiException("STORE_NOT_ACTIVE", "A loja deve estar com status Ativa antes de iniciar a implantação.");
}
function setPlannedOpeningDateV1(spreadsheet, user, payload) {
    ...
    return withScriptLock(() => {
        ...
        if (previous === plannedDate)
            return { store: mapImplantationStoreV1(stores, found.current), idempotent: true };
        const now = new Date();
...
function startImplantationV1(spreadsheet, user, payload) {
    ...
    return withScriptLock(() => {
        ...
        const cycles = readTable(spreadsheet, APP_CONFIG.sheets.storeImplantations, ["ID_Implantacao", "ID_Loja", "Status_Ciclo", "version", "ativo"]);
        if (currentImplantationForStoreV1(cycles, storeId))
            throw new ApiException("IMPLANTATION_EXISTS", "Esta loja já possui uma implantação ativa.");
        ...
        setCell(cycles, cycleRow, "ID_Implantacao", implantationId);
        setCell(cycles, cycleRow, "ID_Loja", storeId);
        setCell(cycles, cycleRow, "ID_Modelo_Versao", modelId);
        setCell(cycles, cycleRow, "ID_Usuario_Coordenador", user.id);
        setCell(cycles, cycleRow, "Data_Inauguracao_Base", openingDate);
        setCell(cycles, cycleRow, "Data_Inauguracao_Planejada_Atual", openingDate);
        setCell(cycles, cycleRow, "Status_Ciclo", "Ativo");
        setCell(cycles, cycleRow, "Iniciada_Em", now);
        setCell(cycles, cycleRow, "Iniciada_Por", user.id);
        setCell(cycles, cycleRow, "ativo", "Sim");
        setTechnicalCreationFields(cycles, cycleRow, user, now);
...
function buildStartedActivityRowV1(target, template, source, id, implantationId, storeId, modelVersion, openingDate, user, now) {
    ...
    setCell(target, row, "ID_Checklist_Loja", id);
    setCell(target, row, "ID_Implantacao", implantationId);
    setCell(target, row, "ID_Loja", storeId);
    ...
    setCell(target, row, "Data_Alvo_Original", targetDate);
    setCell(target, row, "Data_Alvo_Atual", targetDate);
    setCell(target, row, "Status", implantationStatusLabelV1("NAO_INICIADO"));
    setCell(target, row, "Percentual_Concluido", 0);
    setCell(target, row, "Ultima_Atualizacao_Em", now);
    setCell(target, row, "ativo", "Sim");
    setTechnicalCreationFields(target, row, user, now);
    return row;
}
function buildImplantationUpdateRowV1(table, id, values, user, now) {
    const row = Array(table.headers.length).fill("");
    setCell(table, row, "ID_Atualizacao", id);
    setCell(table, row, "ID_Checklist_Loja", values.activityId);
    setCell(table, row, "ID_Implantacao", values.implantationId);
    setCell(table, row, "ID_Loja", values.storeId);
    setCell(table, row, "Tipo_Atualizacao", values.type);
    setCell(table, row, "Texto", values.text);
    setCell(table, row, "Status_Anterior", values.previousStatus);
    setCell(table, row, "Status_Novo", values.nextStatus);
    setCell(table, row, "Progresso_Anterior", values.previousProgress);
    setCell(table, row, "Progresso_Novo", values.nextProgress);
    setCell(table, row, "ID_Responsavel_Anterior", values.previousResponsible);
    setCell(table, row, "ID_Responsavel_Novo", values.nextResponsible);
    setCell(table, row, "Data_Hora", now);
    setCell(table, row, "ID_Usuario", user.id);
    setCell(table, row, "Origem", "SISTEMA_WEB");
    setCell(table, row, "Request_ID", values.requestId);
    setCell(table, row, "ativo", "Sim");
    setTechnicalCreationFields(table, row, user, now);
    return row;
}
function updateImplantationActivityV1(spreadsheet, user, payload) {
    return mutateImplantationActivityV1(spreadsheet, user, payload, "UPDATE", false);
}
function markImplantationActivityNotApplicableV1(spreadsheet, user, payload) {
...
function mutateImplantationActivityV1(spreadsheet, user, payload, updateType, canCancel, canReopen = false, additionalImplantationPermission = "") {
    ...
    return withScriptLock(() => {
        ...
        if (updateType === "Reabertura") {
            if (["CONCLUIDO", "NAO_APLICAVEL", "CANCELADO"].indexOf(previousStatus) < 0)
                throw new ApiException("INVALID_TRANSITION", "Somente atividades encerradas podem ser reabertas.");
        ...
        if (nextResponsible)
            assertEligibleImplantationResponsibleV1(spreadsheet, nextResponsible, storeId);
        const changes = [];
        if (targetStatus !== previousStatus)
            changes.push({ field: "Status", previous: previousStatus, next: targetStatus });
        if (nextProgress !== previousProgress)
            changes.push({ field: "Percentual_Concluido", previous: previousProgress, next: nextProgress });
        if (nextResponsible !== previousResponsible)
            changes.push({ field: "ID_Usuario_Responsavel", previous: previousResponsible, next: nextResponsible });
        if (reason && reason !== cell(activities, found.current, "Ultima_Observacao"))
            changes.push({ field: "Ultima_Observacao", previous: cell(activities, found.current, "Ultima_Observacao"), next: reason });
        if (!changes.length)
            throw new ApiException("VALIDATION_ERROR", "Nenhuma alteração válida foi informada.");
        const now = new Date();
        ...
        setCell(activities, found.current, "ID_Usuario_Responsavel", nextResponsible);
        if (reason)
            setCell(activities, found.current, "Ultima_Observacao", reason);
        if (previousStatus === "NAO_INICIADO" && targetStatus === "EM_ANDAMENTO")
            setCell(activities, found.current, "Data_Inicio_Real", formatDateOnly(now));
        if (targetStatus === "CONCLUIDO")
            setCell(activities, found.current, "Data_Conclusao_Real", formatDateOnly(now));
        if (targetStatus !== "CONCLUIDO" && previousStatus === "CONCLUIDO")
            setCell(activities, found.current, "Data_Conclusao_Real", "");
        ...
        setCell(activities, found.current, "updated_at", now);
        setCell(activities, found.current, "updated_by", user.id);
...
function assertEligibleImplantationResponsibleV1(spreadsheet, userId, storeId) {
...
function blockImplantationActivityV1(spreadsheet, user, payload) {
    assertModulePermission(spreadsheet, user, IMPLANTATION_UPDATES_MODULE_V1, "Criar");
    ...
    return withScriptLock(() => {
        ...
        const now = new Date();
        setCell(activities, found.current, "Status", implantationStatusLabelV1("BLOQUEADO"));
        setCell(activities, found.current, "Ultima_Observacao", reason);
        setCell(activities, found.current, "Ultima_Atualizacao_Em", now);
        setCell(activities, found.current, "version", found.currentVersion + 1);
        setCell(activities, found.current, "updated_at", now);
        setCell(activities, found.current, "updated_by", user.id);
        const blockId = nextInternalId(blocks, "ID_Bloqueio", "BLQ", 6);
        const blockRow = Array(blocks.headers.length).fill("");
        setCell(blocks, blockRow, "ID_Bloqueio", blockId);
        setCell(blocks, blockRow, "ID_Checklist_Loja", activityId);
        setCell(blocks, blockRow, "ID_Implantacao", cell(activities, found.current, "ID_Implantacao"));
        setCell(blocks, blockRow, "ID_Loja", storeId);
        ...
        const activityRange = activities.sheet.getRange(physicalRowNumber(activities, found.rowIndex), 1, 1, activities.headers.length);
        const blockStart = findFirstWritableRow(blocks, "ID_Bloqueio");
        const blockRange = blocks.sheet.getRange(blockStart, 1, 1, blocks.headers.length);
        const updateStart = findFirstWritableRow(updates, "ID_Atualizacao");
        const updateRange = updates.sheet.getRange(updateStart, 1, 1, updates.headers.length);
        performAtomicWritesV1(spreadsheet, user, [
            { range: activityRange, previous: restorableMatrixV1(activityRange), next: [found.current] },
...
function unblockImplantationActivityV1(spreadsheet, user, payload) {
    assertModulePermission(spreadsheet, user, IMPLANTATION_UPDATES_MODULE_V1, "Criar");
    const activityId = requireString(payload.activityId, "activityId");
    const expectedVersion = Number(payload.version);
    const requestId = requireImplantationRequestIdV1(payload);
    ...
    return withScriptLock(() => {
        ...
        if (blockIndex < 0)
            throw new ApiException("ACTIVE_BLOCK_NOT_FOUND", "O bloqueio ativo não foi encontrado.");
        const block = blocks.rows[blockIndex].slice();
        const blockVersion = Number(cell(blocks, block, "version") || 1);
        ...
        setCell(activities, found.current, "updated_at", now);
        setCell(activities, found.current, "updated_by", user.id);
        setCell(blocks, block, "Data_Desbloqueio", now);
        setCell(blocks, block, "ID_Usuario_Desbloqueio", user.id);
        setCell(blocks, block, "Observacao_Desbloqueio", observation);
        setCell(blocks, block, "version", blockVersion + 1);
        setCell(blocks, block, "updated_at", now);
        setCell(blocks, block, "updated_by", user.id);
        const updateId = nextInternalId(updates, "ID_Atualizacao", "ATU", 6);
        const updateRow = buildImplantationUpdateRowV1(updates, updateId, {
...
function previewOpeningDateChangeV1(spreadsheet, user, payload) {
    assertModulePermission(spreadsheet, user, IMPLANTATION_MODULE_V1, "Editar");
    const storeId = requireString(payload.storeId, "storeId");
    const newDate = validateOpeningDateV1(payload.plannedOpeningDate);
    assertStoreScope(user, storeId);
    const cycles = readTable(spreadsheet, APP_CONFIG.sheets.storeImplantations, ["ID_Implantacao", "ID_Loja", "Status_Ciclo", "version", "ativo"]);
    const cycle = currentImplantationForStoreV1(cycles, storeId);
    if (!cycle)
        throw new ApiException("IMPLANTATION_NOT_STARTED", "A loja ainda não possui implantação iniciada.");
...
function buildOpeningDateImpactsV1(table, rows, newDate) {
    return rows.flatMap((row) => {
        const status = implantationStatusCodeV1(cell(table, row, "Status"));
        if (["CONCLUIDO", "NAO_APLICAVEL", "CANCELADO"].indexOf(status) >= 0)
            return [];
        return [{
...
function changePlannedOpeningDateV1(spreadsheet, user, payload) {
    assertModulePermission(spreadsheet, user, IMPLANTATION_MODULE_V1, "Editar");
    const storeId = requireString(payload.storeId, "storeId");
    const newDate = validateOpeningDateV1(payload.plannedOpeningDate);
    const implantationVersion = Number(payload.implantationVersion);
    const storeVersion = Number(payload.storeVersion);
    const reason = requireString(payload.reason, "reason");
    const requestId = requireImplantationRequestIdV1(payload);
    assertStoreScope(user, storeId);
    return withScriptLock(() => {
        ...
        if (!cycleRow)
            throw new ApiException("IMPLANTATION_NOT_STARTED", "A implantação não foi encontrada.");
        ...
        const activities = readTable(spreadsheet, APP_CONFIG.sheets.implantationActivities, ["ID_Checklist_Loja", "ID_Implantacao", "Status", "Data_Alvo_Atual", "version", "ativo"]);
        const rows = activeActivitiesForImplantationV1(activities, cycleId);
        const impacts = buildOpeningDateImpactsV1(activities, rows, newDate);
        const versionById = isRecord(payload.activityVersions) ? payload.activityVersions : {};
        impacts.forEach((impact) => {
            if (Number(versionById[impact.activityId]) !== impact.version)
        ...
        setCell(stores, storeFound.current, "updated_at", now);
        setCell(stores, storeFound.current, "updated_by", user.id);
        setCell(cycles, cycleFound.current, "Data_Inauguracao_Planejada_Atual", newDate);
        setCell(cycles, cycleFound.current, "version", cycleFound.currentVersion + 1);
        setCell(cycles, cycleFound.current, "updated_at", now);
        setCell(cycles, cycleFound.current, "updated_by", user.id);
        const writes = [];
        const audits = [];
        ...
        const impactById = {};
        impacts.forEach((impact) => { impactById[impact.activityId] = impact; });
        const changedRows = [];
        const changedPhysical = [];
        rows.forEach((row) => {
            const impact = impactById[cell(activities, row, "ID_Checklist_Loja")];
            if (!impact || impact.previousTargetDate === impact.nextTargetDate)
                return;
            const index = activities.rows.indexOf(row);
            const next = row.slice();
            setCell(activities, next, "Data_Alvo_Atual", impact.nextTargetDate);
            setCell(activities, next, "Ultima_Observacao", reason);
            setCell(activities, next, "Ultima_Atualizacao_Em", now);
            setCell(activities, next, "version", impact.version + 1);
            setCell(activities, next, "updated_at", now);
            setCell(activities, next, "updated_by", user.id);
            changedRows.push(next);
            changedPhysical.push(physicalRowNumber(activities, index));
